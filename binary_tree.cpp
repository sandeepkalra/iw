...
struct node * FindCommonAncestorByPointer(struct node* r, struct node*a , struct node*b)
{
	 if (r==nullptr) return nullptr;
	 if (r->left == a || r->right == b || r->right == a || r->left == b) return r;
	 struct node * lca_l=0, *lca_r=0;
	 if (r->left) lca_l = FindCommonAncestorByPointer(r->left, a, b);
	 if (r->right) lca_r = FindCommonAncestorByPointer(r->right , a, b);
	 if (lca_l && lca_r ) return r;
